...
class Config:
    def __init__(self):
        ...
        self.spray_pattern = load_spray_pattern(f"{self.selected_weapon}.txt")

        self.recoil_random_range = 2

        self.recoil_steps = 10
        self.recoil_step_delay = 0.01
...
# === DROPDOWN: Weapon selection ===
    ...
    weapon_options = ["ak", "m4a4", "m4s", "galil"]
    weapon_var = tk.StringVar(value=config.selected_weapon)

    def on_weapon_select(choice):
        config.selected_weapon = choice
        try:
            config.spray_pattern, config.recoil_steps, config.recoil_step_delay = load_spray_pattern(f"{choice}.txt")
            print(f"[RECOIL] Loaded {choice}.txt with {config.recoil_steps=} and {config.recoil_step_delay=}")
    ...
    tk.Scale(recoil_tab, from_=1, to=30, orient=tk.HORIZONTAL, command=update_recoil_steps).pack()
    ...
    def save_pattern_to_file():
        update_pattern_from_entries()
        with open(f"{config.selected_weapon}.txt", "w") as f:
            f.write(str(config.spray_pattern))
        print(f"[RECOIL] Pattern saved to {config.selected_weapon}.txt")
...
def main():
    ...
    #cv2.destroyAllWindows()  # DEBUGGING WINDOW
                 
if __name__ == "__main__":
    main()

    # CSGO-fadvy-Wi8sS-yS3FO-sj7dY-wHH8D