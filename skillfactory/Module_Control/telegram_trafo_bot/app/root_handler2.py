...
async def db_table(message: Message):
    await message.answer("T.Add")

@root_router.message(F.text == 'T.List')
async def db_table(message: Message):
    await message.answer("T.List")
    ...
    if conn is not None:
        try:
            ...
            with conn.cursor() as curs:
                ...
                await message.answer(f"Список таблиц в {db_name}:")
                for table in tables:
                    print(table[0])
                    await message.answer(f"{table[0]}")
        except Exception as ex:
            print(f"Error in db_list: {ex}")
    else:
        await message.answer("Connection to the database is not established.")
