...
def is_rect_collision(obj1, obj2):
    # Проверяем, пересекаются ли обьекты (прямоугольники)
    # находится ли левая граница первого обьекта левее правой границы второго обьекта
    # находится ли правая граница первого обьекта правее левой границы второго обьекта
    # находится ли верхняя граница первого обьекта выше нижней границы второго обьекта
    # находится ли нижняя граница первого обьекта ниже верхней границы второго обьекта
    ...
    while True:
        try:
            ...
            for player_id, player_obj in player_list.items():
                if player_id != current_id and is_rect_collision(player_data, player_obj):
                    print(f"Коллизия игрока {current_id} с игроком {player_id}")

            for tree in tree_list:
                if is_rect_collision(player_data, tree):
                    print(f"Коллизия игрока {current_id} с деревом {tree.id}")

            for ore in ore_list:
                if is_rect_collision(player_data, ore):
                    print(f"Коллизия игрока {current_id} с рудой {ore.id}")
